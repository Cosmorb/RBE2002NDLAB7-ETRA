...
void LSM6::readAcc(void)
{
  ...
  a.x = (int16_t)(xha << 8 | xla) - offset_A_X; //Subtract the offset from the accelerometer readings
...
void LSM6::readGyro(void)
{
  ...
  g.z = (int16_t)(zhg << 8 | zlg) - offset_G_Z;
}
...
void LSM6::calibrate(void)
{
  int i = 0;
  int sample = 200; //Variable to specify number of samples to average
  int32_t average_A_X = 0; //Variable to store the readings of the accelerometer x-axis
  int32_t average_G_Z = 0; //Variable to store the readings of the gyroscope z-axis
  while (i < sample)
  {
    if (checkForNewData()) {
      i++;
      average_A_X += a.x; //Sum the values of the accelerometer
      average_G_Z += g.z; //Sum the values of the gyroscope
    }
  }
  offset_A_X = average_A_X / sample; //Average the values of the accelerometer
  offset_G_Z = average_G_Z / sample; //Average the values of the gyroscope
}